...
export default function CheckoutPage() {
  ...
  if (isSuccess) {
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center pt-20 pb-20 px-6 relative">
        ...
        <motion.div 
          ...
          className="max-w-md w-full mx-auto p-8 rounded-3xl border border-border bg-card/90 text-center shadow-2xl"
        >
          ...
          <h2 className="text-2xl font-serif text-foreground mb-4">تم استلام طلبك بنجاح! 🎉</h2>
          ...
          <div className="p-4 rounded-xl bg-secondary/30 border border-border text-[10px] text-muted-foreground text-right mb-8">
          ...
          <div className="space-y-3">
            <MagneticWrapper>
              <Link href="/dashboard" className="flex items-center justify-center w-full py-3 rounded-xl bg-foreground text-background font-bold text-xs hover:opacity-90 transition-colors shadow-lg">
                متابعة الطلبات ولوحة العميل
              </Link>
            </MagneticWrapper>
            <MagneticWrapper>
              <Link href="/store" className="flex items-center justify-center w-full py-3 rounded-xl bg-secondary border border-border text-foreground font-bold text-xs hover:bg-secondary/80 transition-colors">
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center pt-20 pb-20">
        <div className="text-center">
          ...
          <MagneticWrapper>
            <Link href="/store" className="bg-foreground text-background font-bold text-xs rounded-xl px-5 py-2.5 flex items-center gap-2 justify-center w-fit mx-auto hover:opacity-90 transition-colors shadow-lg">
  ...
  return (
    <div className="min-h-screen bg-background text-foreground pt-32 pb-20 relative">
      ...
      <div className="container mx-auto px-6 max-w-6xl relative z-10">
        <h1 className="text-3xl font-serif mb-2 text-right text-foreground">إكمال عملية الشراء</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="rounded-2xl border border-border bg-card/95 p-6">
                <h3 className="font-bold text-sm mb-5 flex items-center gap-3">
                  <span className="w-6 h-6 rounded-full bg-secondary border border-border text-foreground flex items-center justify-center text-xs font-bold">1</span>
                    ...
                    <button
                      ...
                      className={`p-4 rounded-xl border transition-all text-center flex flex-col items-center justify-center ${
                        selectedMethod === method.id
                          ...
                          : "border-border bg-secondary/20 hover:border-foreground/20 hover:bg-secondary/40"
                      }`}
                    >
                      <span className="text-2xl mb-2 block">{method.icon}</span>
                      <span className="text-xs font-bold block text-foreground">{method.name}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <AnimatePresence mode="wait">
                {currentMethod && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -8 }}
                    className="rounded-2xl border border-border bg-card/95 p-6"
                  >
                    <h3 className="font-bold text-sm mb-5 flex items-center gap-3">
                      <span className="w-6 h-6 rounded-full bg-secondary border border-border text-foreground flex items-center justify-center text-xs font-bold">2</span>
                      إرسال الحوالة والدفع
                    </h3>

                    {/* Payment Number */}
                    <div className="bg-secondary/20 border border-border p-4 rounded-xl mb-4 flex items-center justify-between">
                      <div>
                        <p className="text-[10px] text-muted-foreground mb-1">رقم المحفظة / الحساب للتحويل:</p>
                        <span className="font-mono font-bold text-base text-foreground" dir="ltr">{currentMethod.number}</span>
                      </div>
                      <MagneticWrapper>
                        <button
                          type="button"
                    ...
                    {/* Amount to send */}
                    <div className="bg-secondary/40 border border-border p-4 rounded-xl mb-4 flex items-center justify-between">
                      <div>
                        <p className="text-[10px] text-muted-foreground mb-1">المبلغ المطلوب إرساله بالضبط:</p>
                        <p className="text-xl font-bold text-foreground">{total.toLocaleString()} د.ع</p>
                      </div>
                      <span className="text-[9px] text-muted-foreground font-semibold px-2 py-0.5 bg-secondary border border-border rounded-md">عملة عراقية</span>
                    </div>
                      ...
                      <div className="border-t border-border pt-5">
                        ...
                        {receiptPreview ? (
                          <div className="space-y-4">
                            ...
                            <div className="relative rounded-xl overflow-hidden border border-border bg-black max-w-sm mx-auto">
                              <Image
                                src={receiptPreview}
                                alt="إيصال الدفع"
                                width={400}
                            ...
                            <div className="bg-secondary/20 border border-border rounded-xl p-4 text-right">
                              <h4 className="text-[11px] font-bold text-foreground mb-3 flex items-center gap-1.5 justify-end">
                              ...
                              <ul className="space-y-2.5 text-[10px]">
                                {[
                                  ...
                                  <li key={item.step} className="flex justify-between items-center text-muted-foreground">
                                    ...
                                    <span className={scanStep >= item.step ? "text-foreground font-semibold" : ""}>{item.label}</span>
                                  </li>
                                ))}
                              </ul>
                              ...
                              {scanSuccess && (
                                <div className="mt-4 pt-3 border-t border-border text-[10px] text-emerald-400 leading-normal bg-emerald-500/5 border border-emerald-500/10 p-3 rounded-lg">
                          ...
                          <MagneticWrapper>
                            <button
                              ...
                              className="w-full py-8 rounded-xl border border-dashed border-border hover:border-foreground/20 bg-secondary/10 hover:bg-secondary/20 transition-all flex flex-col items-center gap-3"
                            >
                              <div className="w-10 h-10 rounded-full bg-secondary border border-border flex items-center justify-center">
                                <Upload className="w-5 h-5 text-muted-foreground" />
                              </div>
                              <div className="text-center">
                                <p className="font-semibold text-xs text-foreground">اضغط هنا لرفع صورة إيصال التحويل</p>
              ...
              <MagneticWrapper>
                <button 
                  ...
                  className="w-full py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 bg-foreground text-background hover:opacity-90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-xs shadow-lg"
                >
                  {isProcessing ? (
          ...
          >
            <div className="rounded-2xl border border-border bg-card/95 p-6 sticky top-28">
              <h3 className="font-bold text-sm mb-5 text-right text-foreground">ملخص السلة ({totalCount} منتجات)</h3>
              ...
              <div className="space-y-4 mb-6">
                {items.map(item => (
                  <div key={item.id} className="flex justify-between items-center py-2 border-b border-border">
                    ...
                    <span className="font-bold text-xs text-foreground">{(item.price * item.quantity).toLocaleString()} د.ع</span>
                  </div>
                ))}
              </div>

              <div className="pt-4 border-t border-border flex justify-between items-end">
                <span className="text-lg font-serif text-foreground">الإجمالي</span>
                <span className="text-2xl font-bold text-foreground">{total.toLocaleString()} د.ع</span>
              </div>
              
              <div className="mt-8 flex items-center justify-center gap-2 text-[10px] text-muted-foreground bg-secondary/20 border border-border p-3 rounded-xl">
